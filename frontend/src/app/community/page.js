...
export default function CommunityPage() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      const res  = await fetch(`http://localhost:3001/v1/api/posts?${qs.toString()}`);
      const json = await res.json();                 // paginate result
      setPosts(json.docs || json.results || []);     // adjust to your shape
      setLoading(false);
    };
    fetchPosts();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold text-center mb-8">Anonymous Community</h1>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8">
          {["All", "General Support", "Anxiety & Depression", "Relationship Issues", "Crisis Support"].map((c) => (
            <button
              key={c}
...
<div className="flex justify-end mb-4">
  <Link href="/community/new" className="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">
    Ask a Question
  </Link>
</div>
...
{/* Post list */}
    {loading ? (
          ...
          <ul className="space-y-4">
            {posts.map((p) => (
              <li key={p._id} className="bg-white rounded-lg shadow-md p-4 hover:bg-gray-50 transition-colors">
                <Link href={`/community/post/${p._id}`}>
                  <h3 className="font-semibold text-lg mb-1 hover:text-indigo-600">{p.title}</h3>
                </Link>
                <div className="text-sm text-gray-500">
                  {p.category} • {p.repliesCount || 0} replies • {p.createdAt?.slice(0, 10)}
                </div>
              </li>
            ))}
          </ul>
