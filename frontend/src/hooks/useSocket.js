...
export const useSocket = () => {
  ...
  const onSessionRequest = (callback) => {
    if (socketRef.current) {
  ...
  const onRequestSent = (callback) => {
    if (socketRef.current) {
      socketRef.current.on('request-sent', callback);
    }
  };

  const onRequestFailed = (callback) => {
    if (socketRef.current) {
      socketRef.current.on('request-failed', callback);
    }
  };

  const onRequestDeclined = (callback) => {
    if (socketRef.current) {
